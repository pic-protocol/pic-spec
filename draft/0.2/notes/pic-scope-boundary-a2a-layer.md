...
| Protocol-level policy enforcement and authority reduction: guardrails and PIC Trusted Anchors, with the outcome bound inside the signed advancement | Sandboxed Execution; the PIC-X discovery design article | Work in progress in PIC, as PIC-X itself is. The Trusted Anchor specification is being written; the component is part of the published PIC-X *design*: the design article (Designing PIC-X: .well-known configuration, 1 August 2026) places a `trust_anchors_endpoint` in the realm discovery document and describes PIC Trusted Anchors as protocol-level trust policy engines. The current PIC-X build does not serve that endpoint and therefore does not advertise it — an unserved endpoint must not appear in discovery, so that a client cannot integrate against something that answers 404 — and the same holds for `revocation_endpoint`; `attestations_endpoint` is the one served today. The Prover and Verifier §4.3 places the policy decision with the application (see §3.1); the Trusted Anchor profile moves that boundary and §4.3 will be aligned when it is published |
